...
import {
  ...
  MAX_POSTERIOR_AMPLITUDE,
  NARROW_POSTERIOR_AMPLITUDE,
  POINTER_MEAN_SHIFT,
  POINTER_AMPLITUDE_WIGGLE,
  HERO_TREE_HEIGHT,
  HERO_TREE_RIGHT_WIDTH,
  CANOPY_MAX_WIDTH,
  TERRAIN_COLUMNS,
  clamp,
  dampedSpring,
...
export const initializeHeroPixelFields = () => {
  ...
  document.querySelectorAll<HTMLElement>('[data-hero-pixel-field]').forEach((field) => {
    ...
    const titleInkSurfaces = titleInk
      ...
      : [];
    const clippedTrees = field.dataset.visualComposition === 'badge'
      ? field.querySelector<HTMLElement>('.hero-pixel-field__trees') : null;
    ...
    const collectBranchLayouts = (fieldBox: DOMRect) => {
      if (!treeMotionEnabled || !treeVisible()) {
        branchLayouts = [];
        rightTreeRect = null;
        return;
      }
      ...
      branchLayouts = branchElements.flatMap((element) => {
        ...
        if (!positionBox || positionBox.width === 0 || positionBox.height === 0) return [];
      ...
      // Read all pixel bounds before writing styles, avoiding one forced layout per branch.
      branchLayouts.forEach(({ element, side }) => {
        const sourceWidth = side === 'left' ? 744 : HERO_TREE_RIGHT_WIDTH;
        element.style.transformOrigin = `${(Number(element.dataset.branchAnchorX) / sourceWidth * 100).toFixed(3)}% ${(Number(element.dataset.branchAnchorY) / HERO_TREE_HEIGHT * 100).toFixed(3)}%`;
      });
    };
