...
尚未确认日期的专题和嘉宾只出现在共享专题嘉宾列表中，不得被重复分配到具体日期。
空报告对象和 `Time TBD`、`Talk title TBD`、`Speaker TBD` 等占位行不得进入构建产物。
...
- `src/components/goal/GoalConferenceProgramOutline.astro` 只在 `/goal/schedule/` 渲染三天日程框架。
- `src/components/ConferenceProgramPreview.astro` 是公开日程和 Goal 日程共用的专题嘉宾渲染器。
